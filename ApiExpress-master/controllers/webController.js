...
exports.getMenu = async (req, res) => {
    try {
        ...
        console.log("Problemas al renderizar la página de menú");
...
exports.getProductosDeCantabria = async (req, res) => {
    try {
        const productosdecantabria = await ProductosDeCantabria.find({ esCantabro: true });
        res.render('productosdecantabria', { productosdecantabria });
    } catch (error) {
        console.log("Problemas al renderizar la página de productos de Cantabria:", error);
        res.status(500).send("Error interno del servidor");
    }
};
